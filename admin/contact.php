...
//backup for deleted user contact
    $reset="INSERT INTO del_contact SELECT * FROM contact WHERE id='$ied'";
            mysqli_query($con,$reset);
    
    $sqldta = "DELETE FROM `contact` WHERE id='$ied'";
    mysqli_query($con,$sqldta);
...
#customers th {
	...
	<body  class="bd">
		<div class="view" >
           ...
           $search=$_POST['search'];
           header('location:contact.php?iv='.$search);
  }
  else {
...
?>
   ...
   $sql_profile="select * from contact where (name like '%" . str_replace( ' ', '%\' AND name LIKE \'%', $getresult ) . '%\') 
   ...
   // echo $sql_profile="select * from user_login where country LIKE '%$getresult%'";
    ...
    <tr>
      ...
      <td><a href="contact.php?action=del&id=<?php echo base64_encode($arr_profile['id'])?>" style="color: red">Checked</a></td>
    </tr>
  ...
  <table id="customers">
    ...
    <tr>
     <td><?php echo $arr['id']?></td>
      ...
      <td><a href="mailto:<?php echo $arr['email']?>?subject=Traveler.com &body=Hi <?php echo str_replace('&nbsp',' ', $arr['name'])?> This is a Reply  for Your Message  (<?php echo $arr['ask']?>) "><?php echo $arr['email']?></a></td>
     ...
     <td><a href="contact.php?action=del&id=<?php echo base64_encode($arr['id'])?>" style="color: red">Checked</a></td>
    </tr>
    <?php }?>
  </table>
</div>
