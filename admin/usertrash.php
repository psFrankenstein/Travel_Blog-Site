...
if(!isset($_SESSION['id']))
{
	...
	$query="select * from  del_user ORDER BY id DESC";
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=="del")
	{
		...
		$sql = "delete from del_user where id='$id'";
		mysqli_query($con,$sql);
		
			header("location:usertrash.php");
	}
?>
<!DOCTYPE html>
<html>
...
1px 1px 1px 1px #6f6f6f;
	...
	<body  class="bd">
		<div class="view" >
			<h3><a href="trash.php"><abbr title="Back"><img src="photos/backbutton.png"></abbr></a>Register User's Of The Page</h3>
           ...
           $search=$_POST['search'];
           header('location:usertrash.php?iv='.$search);
  }
  else {
...
?>
   ...
   $sql_profile="select * from del_user where (name like '%" . str_replace( ' ', '%\' AND name LIKE \'%', $getresult ) . '%\') 
   ...
   // echo $sql_profile="select * from del_user where country LIKE '%$getresult%'";
       ...
       <?php while( $arr_profile=mysqli_fetch_assoc($req_profile)){
          $ird=$arr_profile['id'];?>
				...
				<td><?php echo date("y-m-d-H:i:s",$arr_profile['last_seen'])?></td>
					...
					<td ><a href="usertrash.php?action=del&id=<?php echo base64_encode($arr_profile['id'])?>" style="color: red"> Delete</a></td>
				</tr>
			...
			<table id="customers" >
				...
				<td  ><?php echo date("y-m-d-H:i:s",$arr['last_seen'])?></td>
					...
					<td ><a href="usertrash.php?action=del&id=<?php echo base64_encode($arr['id'])?>" style="color: red">Delete</a></td>
				</tr>
				<!--admin view container end here -->
				<?php }?>
